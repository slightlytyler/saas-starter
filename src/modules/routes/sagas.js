...
export function* fetchCollection({ payload }) {
  try {
    ...
    yield compose(put, toastsActions.add)({
      message: 'Fetch route collection failed.',
      type: 'failure',
    });
  } finally {
    if (yield cancelled()) {
...
export function* fetchRecord({ payload, meta: { transactionId } }) {
  try {
    const { body } = yield call(fetchSaga, { endpoint: `/routes/${payload.id}` });
    yield compose(
      put,
    ...
    yield compose(put, toastsActions.add)({
      message: `Fetch route ${payload.id} failed.`,
      type: 'failure',
    });
  } finally {
    if (yield cancelled()) {
